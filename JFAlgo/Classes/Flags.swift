...
//  Pods
//
//  Created by Javier Fuchs on 9/9/16.
//
//
...
public class Flags : Algorithm {
    
    override public init() {
        super.init()
        input = Limit(min: 1, max: 400000)
        domain = Limit(min: 0, max: 1000000000)
    }
    ...
    public override func solution(inout A : [Int]) -> Int {
        ...
        /// Flags can only be set on peaks. What's more, if you take K flags, then the distance between any two flags should be greater than or equal to K. The distance between indices P and Q is the absolute value |P − Q|.
        let K = peaks.count
        for Q in 1..<peaks.count {
            let P = Q - 1
            if abs(peaks[P] - A[Q]) >= K {
                flags.append(A[Q])
            }
        }
        return flags.count
